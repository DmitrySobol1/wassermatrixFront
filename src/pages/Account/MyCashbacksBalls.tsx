    ...
    <Page back={true}>
      ...
      {!isLoading && (
        <>
          ...
          >
            <Cell
            after={`${cashbackBall} ${userValute}`}
            >
                 {cashbackBallT}
            </Cell>
            <Cell
            after={`${purchaseQty} ${pcsT}`}
            >
                 {purchaseQtyT}
            </Cell>
            <Cell after={`${purchaseSum} ${userValute}`}>
                 {purchaseSumT}
            </Cell>
            <Cell
            after={`${clientCashbackLevel} (${currentPercent}%)`}
            >
              {clientCashbackLevelT}
            </Cell>
            <Cell
            after={deltaToNextLevel > 0 ? `${deltaToNextLevel} ${userValute}` : maxLevelT}
            >
              {deltaToNextLevelT} 
            </Cell>
